...
def data_bace():
    db = mysql.connector.connect(host="localhost", user="root", password="admin", database="dermatology")
    cur=db.cursor()
    return db,cur
...
def upload():
    ...
    if request.method=='POST':
        ...
        if fn.split('.')[-1] not in accepted_formated:
            ...
            return render_template("upload.html")
        new_model = load_model("models/mobilenet_fixed.h5")
        test_image = image.load_img(mypath, target_size=(224, 224), color_mode="rgb")  # ✅ force RGB
        test_image = image.img_to_array(test_image)  # Shape: (224, 224, 3)
...
def expertchat(email=""):
    ...
    if request.method=="POST":
        try:
            ...
            val = (useremail,email,date,timeStamp,messages,useremail,session.get('username', 'Unknown'),email)
            print('Inserting chat:', val)
            cur.execute(sql, val)
            db.commit()
            db.close()
        except Exception as e:
            print('Error inserting chat message:', e)
    # Fetch full conversation between this patient and doctor
    db,cur=data_bace()
    sql_select = "SELECT * FROM chatting WHERE patient_email='"+useremail+"' AND doctor_email='"+email+"' ORDER BY chat_date, chat_time"
